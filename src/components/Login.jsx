...
const Login = ({ setIsModalLogin, setIsModalRegister, setIsModalRecover, handleAuthModal }) => {
    ...
    const onSubmit = async (data) => {
        try {
            // Llama a la función grecaptcha.enterprise.execute para obtener el token de reCAPTCHA
            const token = await grecaptcha.enterprise.execute('6LdS9qIpAAAAAEt_mCBaV0v4Sy9qel4YMi33ldla', {action: 'LOGIN'});
            // Llama a loginUser con los datos del formulario y el token de reCAPTCHA
            await loginUser({ ...data, recaptchaToken: token });
        } catch (error) {
            ...
            userVerify?.isVerify ?
                ...
                (
                    <form method='POST' className='login_form' onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="login_items_container_pass">
